...
class CostOptimizerCLI:
    ...
    def print_section_header(self, text: str, emoji: str = "📋"):
        print(f"\n{Colors.BOLD}{Colors.OKCYAN}{text.upper()}{Colors.ENDC}")
    ...
    def run_complete_analysis(self):
        ...
        try:
            ...
            if variance > 0:
                print(f"\n{Colors.WARNING}OVER BUDGET by ₹{variance:,.2f} ({(variance/budget)*100:.1f}%){Colors.ENDC}")
            else:
    ...
    def view_current_data(self):
        ...
        if not has_data:
            print(f"\n{Colors.WARNING}No data loaded yet{Colors.ENDC}")
    ...
    def load_existing_project(self):
        ...
        try:
            ...
            if not loaded_items:
                print(f"{Colors.WARNING}No existing project files found in '{self.outputs_dir}/' directory{Colors.ENDC}")
            else:
